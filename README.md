...
当前配置文件仍兼容 `code-agent-plusplus.config.yml` / `code-agent-plusplus.local.yml`，旧 CLI 命令 `capp` / `code-agent-plusplus` 也会继续作为迁移别名保留。
...
当前 `opencode-plusplus` 还没有发布到 npm，不能直接 `npm i -g opencode-plusplus`。先用源码安装 OpenCode++，并全局链接 `ocpp`：
...
```bash
ocpp          # 进入 OpenCode 聊天模式，自动启用 OpenCode++ sidecar
ocpp report   # 查看最近一次检查结果
ocpp status   # 查看 sidecar 是否 active
ocpp doctor   # 诊断 OpenCode / auth / git / context / plugin
ocpp --pure   # 纯 OpenCode，不启用 OpenCode++

capp          # legacy alias
code-agent-plusplus build .   # legacy alias
```

`ocpp` 会执行 preflight，确保 `.agent-context`，写入 `.opencode/plugins/opencode-plusplus.ts`，准备 OpenCode commands/agent，先打印 3 行短状态，然后进入当前仓库的 OpenCode TUI。Sidecar plugin 会监听 `tool.execute.before`、`tool.execute.after`、`file.edited` 和 `session.idle`：执行危险命令、幻觉 package script / Makefile target、触碰 protected / secret path 时会前置阻断；工具执行结束后会记录 command、exit code、stdout/stderr hash、working tree hash 和 touched files；OpenCode 空闲且有 dirty diff 时会自动运行增量验证，写入 `.agent-context/sidecar/latest.json` 和 `.agent-context/sidecar/latest.md`。

## 高级用法

首页主路径只推荐 `ocpp`。批处理 Harness Mode、CI-like executor、手动 `verify / policy / impact`、MCP 和 retrieval 等内核能力都保留给高级用户：
...
| 能力                                             | 当前状态         | 说明                                                                                 |
| ------------------------------------------------ | ---------------- | ------------------------------------------------------------------------------------ |
| `ocpp` OpenCode TUI launcher                     | MVP              | 已可 preflight、打印短状态、启动 OpenCode TUI，并支持 `--pure`                       |
| OpenCode transparent sidecar plugin              | MVP              | 已注入 `.opencode/plugins/opencode-plusplus.ts`，监听 session / file / tool 事件     |
...
| `ocpp report/status/doctor`                      | Foundation       | 已可读取 sidecar 报告、检查 active 状态、诊断 OpenCode / auth / git / context        |
| batch OpenCode executor / `ocpp oc`              | Foundation       | 适合 benchmark、CI-like run、非交互任务和可重复 demo                                 |
