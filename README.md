...
当前实现支持两种路径：Agent 主导时，它是 Context / Policy / Trace 报告系统 + 显式 runtime 状态机；Code Agent++ 主导时，`orchestrate` 会调用可替换 executor，按 `--max-loops` 执行有边界的 `pack -> execute -> evaluate -> repair/repack/finalize` loop，并把每轮证据写入 `.agent-context/runs/<task-id>/iterations/`。它仍不替代外部 Code Agent 写代码，也不是完整自主 coding agent；它负责上下文、约束、证据、门禁、循环决策报告和最终报告，真实代码修改由外部 executor 执行。
...
Code Agent++ 不直接替代 Agent 写代码，而是通过 Executor Adapter 外挂到不同 Code Agent。当前 executor 支持度见后文成熟度表：
...
关键原则：**Code Agent 可以自主改代码，但不能只靠自然语言宣布完成。** 在 harness-led loop 中，Code Agent++ 会根据证据、diff、测试和策略门禁生成 `finalize / repair / repack / block / require-human-review` 决策报告；最终是否合并仍由用户或 CI 工作流决定。
...
- repair / repack / rerun tests / finalize 决策报告。
...
负责控制 repair/finalize 闭环。它会根据策略、测试、影响分析和证据验证结果，生成下一步应该 finalize、rerun tests、repair code、repair tests、repack context、block、rollback 或 require human review 的决策报告。
...
| 状态         | 含义                                                    |
| ------------ | ------------------------------------------------------- |
| Stable       | 默认推荐使用，已有测试覆盖和 CLI 文档。                 |
| Foundation   | 基础能力可用，接口稳定性较高，但真实项目覆盖仍在扩展。  |
| MVP          | 最小可用版本，适合试用和反馈，规则/数据源还会继续增强。 |
| Experimental | 能跑通但 API、输出和集成方式仍可能变化。                |
| Planned      | 路线图目标，README 不把它当作已完成能力宣传。           |
...
| Context / Boundary / Evidence / Regression Benchmark | Foundation |

Executor Adapter 成熟度：

| Executor / Adapter                   | 成熟度     | 说明                                                                             |
| ------------------------------------ | ---------- | -------------------------------------------------------------------------------- |
| mock executor                        | Stable     | 默认 benchmark / demo executor。                                                 |
| generic `--executor-command` adapter | Foundation | 可调用 OpenCode、Codex、Claude Code、Cursor、MiMoCode 等可脚本化 CLI。           |
| OpenCode event normalizer            | Foundation | 支持 `opencode run --format json` stdout、transcript 和 stdout/stderr fallback。 |
| MiMoCode native normalizer           | Planned    | 计划接入原生事件格式。                                                           |
| Codex JSONL normalizer               | Planned    | 当前可通过 generic command adapter 调用。                                        |
| Claude Code transcript normalizer    | Planned    | 当前可通过 generic command adapter 调用。                                        |
| Cursor native adapter                | Planned    | 当前以文档/MCP/通用命令接入为主。                                                |

MCP / RAG 成熟度：

| 能力                                 | 成熟度       | 说明                                                                                        |
| ------------------------------------ | ------------ | ------------------------------------------------------------------------------------------- |
| MCP stdio server + core tools        | Foundation   | `build/plan/pack/retrieve/tests/impact/verify/explain` 等基础工具可用。                     |
| MCP Agent Native Runtime tools       | Experimental | `start_loop/step/evaluate/repair/finalize` 已有结构化字段，但端到端客户端集成仍需逐个验证。 |
| RAG export / retriever provider 接口 | Foundation   | 可导出 RAG 文档并通过 provider 接口检索。                                                   |
| direct LightRAG server sync          | Planned      | 直接同步到 LightRAG server 仍在路线图中。                                                   |
...
Code Agent++ 的定位是面向 code agent 的 bounded harness-led control loop。Codex / Claude Code / Cursor / OpenCode / MiMoCode 负责实际读代码、改代码和跑命令；Code Agent++ 负责任务上下文、编辑边界、执行证据、门禁、循环决策报告和最终报告。
...
Code Agent++
  -> 负责 context、boundary、trace、policy、impact、tests、verify、repair/finalize 决策报告
```

项目支持两种工作模式：

- Code Agent 主导，Code Agent++ 约束：Agent 调用 CLI / MCP 工具，但最终是否遵守 gate 由宿主 Agent 决定。
- Code Agent++ 主导，Code Agent 作为 executor：Code Agent++ 负责 plan / pack / execute / collect evidence / policy / verify / decision report，外部 Agent 作为可替换编码执行器；这是有边界的 harness-led loop，不是完整自主 coding agent。
...
- Harness gate decisions = produced by Code Agent++；真实修改仍由外部 executor 完成。

## MCP / Agent Native Runtime

`code-agent-plusplus-mcp` 当前提供基础版 stdio MCP server 和 core tools。它已经可以被支持 MCP 的客户端或自研 Agent 接入；Agent Native Runtime tools 仍是实验能力，Codex CLI、Claude Code、Cursor、OpenCode、MiMoCode、LibreChat、OpenHands 等端到端集成仍按客户端逐个验证。
