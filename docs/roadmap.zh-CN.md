...
- 维护 `opencode-plusplus` npm 包，让外部用户可以直接安装试用。
...
`opencode-plusplus` 已发布到 npm。后续发布前必须通过：
...
CI 已覆盖同一组基础检查，`prepublishOnly` 也会执行同一组发布门禁。当前首页安装路径为：
...
详细步骤见 [Release Checklist](release.md)。
