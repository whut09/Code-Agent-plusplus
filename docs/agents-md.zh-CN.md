...
`AGENTS.md` 是给 AI 编程 Agent 阅读的普通 Markdown 指南。Repo-to-Agent-Context 默认把它生成为很短的操作约束文件，只放必须遵守的规则、入口、命令和生成上下文索引。
...
本地 LLM 配置应该由 Agent 代用户处理。真实凭证只写入 `repo-context.local.yml`，该文件已被 git 忽略。如果当前 AI 工具没有把自身模型暴露成可调用 API，Agent 应该先向用户索取 provider、base URL、model 和 key，再运行 `--llm`。
