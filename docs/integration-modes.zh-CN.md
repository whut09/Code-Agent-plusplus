...
Repo-to-Agent-Context 支持两套互不混用的流程。区别不是“能不能用 AI”，而是谁拥有最高控制权。

## 结论

| 模式                                                 | 主控方                                             | 入口                                                                                                       | 是否执行 code agent                                       | 适合场景                                                                |
| ---------------------------------------------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- | ----------------------------------------------------------------------- |
| Code Agent 主导，Repo-to-Agent-Context 约束          | Codex / Claude Code / Cursor / OpenCode / MiMoCode | CLI 的 `plan` / `pack` / `run` / `tests` / `impact` / `verify` / `policy`，或 MCP 的 `repo_context_*` 工具 | 否，由外部 code agent 自己执行                            | 日常 AI 编程、MCP demo、让已有 Agent 自己调用工具                       |
| Repo-to-Agent-Context 主导，Code Agent 作为 executor | Repo-to-Agent-Context                              | `repo-context orchestrate` 或 `repo-context agent run`                                                     | 是，通过 `mock` 或 `--executor-command` 调用外部 executor | 需要可审计 gate、CI/自动化、希望本项目决定 finalize/repair/repack/block |
...
这个模式下，Codex / Claude Code / Cursor / OpenCode / MiMoCode 是主执行者。Repo-to-Agent-Context 提供上下文、边界和验证工具，但不拥有最终执行权。
...
用户任务
  ...
  -> Repo-to-Agent-Context 返回约束、证据和建议
```
...
## 模式二：Repo-to-Agent-Context 主导，Code Agent 作为 executor

这个模式下，Repo-to-Agent-Context 拥有流程编排和验收权。Code agent 只是可替换的编码执行器。
...
- `{prompt}`：Repo-to-Agent-Context 写出的 executor prompt 文件路径。
- `{task}`：原始任务描述。
- `{repo}`：仓库根目录。
...
- 想让 Repo-to-Agent-Context 掌握验收权，并把 code agent 当成编码工具：用模式二。
