export const OPENCODE_PLUSPLUS_PLUGIN_PATH = ".opencode/plugins/opencode-plusplus.ts";
export const LEGACY_OPENCODE_SIDECAR_PLUGIN_PATH = ".opencode/plugins/code-agent-plusplus.ts";
export const OPENCODE_SIDECAR_PLUGIN_PATH = OPENCODE_PLUSPLUS_PLUGIN_PATH;

export function opencodeSidecarPluginTemplate(): string {
  return String.raw`/**
 * OpenCode++ OpenCode Sidecar Plugin
 *
 * Generated by \`ocpp\`. Edit only when you want to customize how OpenCode
...
export const CodeAgentPlusPlusSidecar = async ({ directory, worktree, client }) => {
  ...
  function runCommandGuard(tool, args) {
    ...
    const check = spawnSync("ocpp", cliArgs, {
      cwd: directory,
      encoding: "utf8",
      shell: process.platform === "win32"
    });
  ...
  function recordToolAfter(input, output) {
    try {
      ...
      const recordResult = spawnSync("ocpp", cliArgs, {
        cwd: directory,
        encoding: "utf8",
        shell: process.platform === "win32",
  ...
  function maybeVerifyOnIdle() {
    ...
    dirty = false;
    try {
      const verify = spawnSync("ocpp", ["sidecar", "verify", directory, "--quiet"], {
        cwd: directory,
        encoding: "utf8",
        shell: process.platform === "win32"
      });
      ...
      if ((verify.status ?? 1) !== 0) {
        const output = (verify.stdout || verify.stderr || "OpenCode++ sidecar found blockers. Run ocpp report.").trim();
