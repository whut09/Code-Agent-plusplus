...
Type: bugfix
Budget: 9,949 / 12,000 estimated tokens
...
| Bucket | Tokens | Files |
| --- | --- | --- |
| Directly relevant source files | 2,371 | `src/outputs/regression-guard.ts`, `src/outputs/orchestrator.ts`, `src/outputs/policy-engine.ts`, `src/benchmarks/agent-benchmark.ts`, `src/outputs/task-harness.ts`, `src/outputs/task-context.ts`, `src/integrations/codegraph.ts` |
...
| Dependency neighbors | 5,027 | `src/outputs/task-run.ts`, `src/outputs/writer.ts`, `src/outputs/test-selector.ts`, `src/outputs/guard-finding.ts`, `src/outputs/markdown.ts`, `src/core/types.ts`, `src/core/git.ts`, `src/outputs/execution-trace.ts`, `src/outputs/loop-controller.ts`, `src/outputs/impact.ts`, `src/outputs/agent-events.ts`, `src/outputs/hallucination-guard.ts`, `src/core/context-builder.ts`, `src/core/safe-command.ts`, `src/core/freshness.ts`, `src/benchmarks/benchmark.ts`, `src/outputs/contract-validator.ts`, `src/outputs/evidence.ts`, `src/retrievers/codegraph.ts`, `src/core/token-estimator.ts`, `src/retrievers/types.ts`, `src/sandbox/git-worktree-sandbox.ts`, `src/sandbox/sandbox-adapter.ts`, `src/sandbox/host-sandbox.ts` |
...
Remaining budget: 2,051 estimated tokens
