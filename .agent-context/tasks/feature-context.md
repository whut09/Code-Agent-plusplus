...
Type: feature
Budget: 2,568 / 12,000 estimated tokens
...
4. `src/outputs/contract-validator.ts` - key file fallback; defines ContractValidationOptions
5. `src/core/types.ts` - key file fallback; defines AgentTarget
6. `src/outputs/markdown.ts` - key file fallback; defines heading
7. `src/benchmarks/benchmark.ts` - key file fallback; defines BenchmarkTaskDefinition
...
| Bucket | Tokens | Files |
| --- | --- | --- |
| Directly relevant source files | 1,889 | `src/outputs/task-harness.ts`, `src/core/token-estimator.ts`, `src/core/context-builder.ts`, `src/outputs/contract-validator.ts`, `src/core/types.ts`, `src/outputs/markdown.ts`, `src/benchmarks/benchmark.ts`, `src/outputs/task-context.ts`, `src/outputs/writer.ts` |
| Tests | 0 | none |
...
Remaining budget: 9,432 estimated tokens
