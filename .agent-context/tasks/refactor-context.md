...
Type: refactor
Budget: 11,989 / 12,000 estimated tokens
...
| `src/cli/index.ts` | entrypoint | 309 | direct importer of src/core/types.ts, entrypoint, shared API/refactor risk | src/cli/index.ts contains 102 detected symbols, 31 imports, 0 exports. |
...
| `src/outputs/policy-engine.ts` | dependency-neighbor | 271 | direct dependency of src/mcp/server.ts, direct importer of src/core/types.ts, shared API/refactor risk | src/outputs/policy-engine.ts contains 48 detected symbols, 11 imports, 9 exports. |
| `src/outputs/task-harness.ts` | dependency-neighbor | 280 | direct dependency of src/mcp/server.ts, direct importer of src/core/types.ts, shared API/refactor risk | src/outputs/task-harness.ts contains 68 detected symbols, 8 imports, 5 exports. |
| `src/outputs/task-run.ts` | dependency-neighbor | 203 | direct dependency of src/mcp/server.ts, direct importer of src/core/types.ts, shared API/refactor risk | src/outputs/task-run.ts contains 43 detected symbols, 11 imports, 4 exports. |
...
| `src/core/context-builder.ts` | dependency-neighbor | 106 | direct dependency of src/mcp/server.ts, direct importer of src/core/types.ts, shared API/refactor risk | src/core/context-builder.ts contains 15 detected symbols, 11 imports, 2 exports. |
| `src/outputs/writer.ts` | dependency-neighbor | 246 | direct dependency of src/mcp/server.ts, direct importer of src/core/types.ts, shared API/refactor risk | src/outputs/writer.ts contains 55 detected symbols, 19 imports, 2 exports. |
...
| `src/outputs/task-context.ts` | dependency-neighbor | 260 | direct dependency of src/mcp/server.ts, direct importer of src/core/types.ts, shared API/refactor risk | src/outputs/task-context.ts contains 67 detected symbols, 4 imports, 3 exports. |
...
| `src/outputs/regression-guard.ts` | dependency-neighbor | 297 | direct importer of src/core/types.ts, shared API/refactor risk | src/outputs/regression-guard.ts contains 61 detected symbols, 7 imports, 8 exports. |
...
| `src/outputs/hallucination-guard.ts` | dependency-neighbor | 467 | direct importer of src/core/types.ts, shared API/refactor risk | src/outputs/hallucination-guard.ts contains 106 detected symbols, 6 imports, 8 exports. |
...
| Tests | 2,545 | `test/mcp.test.ts`, `test/agent-events.test.ts`, `test/analyzers.test.ts`, `test/token-savings.test.ts`, `test/readiness.test.ts`, `benchmarks/fixtures/small-ts-app/test/api/login.test.ts`, `benchmarks/fixtures/monorepo/packages/api/test/config.test.ts`, `benchmarks/fixtures/fastapi-app/tests/test_users.py`, `benchmarks/fixtures/react-app/src/auth/useAuthState.test.ts`, `benchmarks/fixtures/react-app/src/components/LoginStatus.test.tsx`, `benchmarks/fixtures/small-ts-app/test/auth/session.test.ts`, `benchmarks/fixtures/monorepo/packages/config/test/loader.test.ts`, `test/context-delta.test.ts`, `test/contract-validator.test.ts`, `test/execution-trace.test.ts`, `test/freshness.test.ts`, `test/hallucination-guard.test.ts`, `test/loop-controller.test.ts`, `test/orchestrator.test.ts`, `test/policy-engine.test.ts`, `test/regression-guard.test.ts`, `test/task-harness.test.ts`, `test/test-selector.test.ts`, `test/agents-md.test.ts`, `test/config.test.ts`, `test/retrievers.test.ts`, `test/writer.test.ts`, `test/cache.test.ts`, `test/impact.test.ts`, `test/task-context.test.ts`, `test/fixtures.test.ts`, `test/validator.test.ts`, `test/scanner.test.ts`, `test/snapshot.test.ts`, `test/benchmark.test.ts` |
| Dependency neighbors | 8,058 | `src/outputs/orchestrator.ts`, `src/outputs/context-delta.ts`, `src/core/token-estimator.ts`, `src/outputs/execution-trace.ts`, `src/outputs/markdown.ts`, `src/outputs/policy-engine.ts`, `src/outputs/task-harness.ts`, `src/outputs/task-run.ts`, `src/outputs/loop-controller.ts`, `src/core/context-builder.ts`, `src/outputs/writer.ts`, `src/outputs/impact.ts`, `src/outputs/test-selector.ts`, `src/outputs/task-context.ts`, `src/retrievers/index.ts`, `src/core/freshness.ts`, `src/outputs/regression-guard.ts`, `src/outputs/contract-validator.ts`, `src/outputs/hallucination-guard.ts`, `src/benchmarks/benchmark.ts`, `src/retrievers/static.ts`, `src/outputs/runtime-state.ts`, `src/outputs/rag.ts`, `src/core/cache.ts`, `src/core/scanner.ts`, `src/config/load-config.ts`, `src/core/indexer.ts`, `src/analyzers/python.ts`, `src/analyzers/javascript.ts`, `src/retrievers/ripgrep.ts`, `src/retrievers/types.ts`, `src/core/token-savings.ts`, `src/core/readiness.ts`, `src/core/validator.ts`, `src/analyzers/types.ts`, `src/core/file-classifier.ts`, `src/outputs/architecture.ts` |
| Config/docs | 0 | none |
| Entrypoints | 648 | `src/mcp/server.ts`, `src/cli/index.ts` |

Remaining budget: 11 estimated tokens
