...
Type: refactor
Budget: 10,332 / 12,000 estimated tokens
...
3. `src/outputs/markdown.ts` - lexical match: code, direct dependency of src/outputs/context-delta.ts; defines heading
4. `src/outputs/context-delta.ts` - lexical match: code, direct importer of src/outputs/markdown.ts; defines DeltaImpactLevel
5. `src/core/token-estimator.ts` - lexical match: code, shared API/refactor risk; defines estimateTokens
6. `src/cli/index.ts` - direct importer of src/outputs/context-delta.ts, direct importer of src/core/token-estimator.ts; defines program
7. `src/mcp/server.ts` - direct importer of src/outputs/context-delta.ts, entrypoint; defines repoContextMcpToolNames
...
- `test/policy-engine.test.ts` - shared API/refactor risk
...
| `src/outputs/markdown.ts` | direct-source | 68 | lexical match: code, direct dependency of src/outputs/context-delta.ts, shared API/refactor risk | src/outputs/markdown.ts contains 8 detected symbols, 0 imports, 5 exports. |
...
| `src/cli/index.ts` | entrypoint | 248 | direct importer of src/outputs/context-delta.ts, direct importer of src/core/token-estimator.ts, entrypoint, shared API/refactor risk | src/cli/index.ts contains 83 detected symbols, 28 imports, 0 exports. |
| `src/mcp/server.ts` | entrypoint | 324 | direct importer of src/outputs/context-delta.ts, entrypoint, shared API/refactor risk | src/mcp/server.ts contains 81 detected symbols, 18 imports, 4 exports. |
...
| `test/execution-trace.test.ts` | test | 72 | shared API/refactor risk | test/execution-trace.test.ts contains 11 detected symbols, 9 imports, 0 exports. |
...
| `test/policy-engine.test.ts` | test | 86 | shared API/refactor risk | test/policy-engine.test.ts contains 15 detected symbols, 10 imports, 0 exports. |
...
| `src/outputs/execution-trace.ts` | dependency-neighbor | 200 | direct importer of src/outputs/markdown.ts, shared API/refactor risk | src/outputs/execution-trace.ts contains 22 detected symbols, 3 imports, 13 exports. |
| `src/outputs/task-run.ts` | dependency-neighbor | 196 | direct importer of src/outputs/markdown.ts, shared API/refactor risk | src/outputs/task-run.ts contains 41 detected symbols, 9 imports, 4 exports. |
...
| `src/outputs/policy-engine.ts` | dependency-neighbor | 242 | direct importer of src/outputs/markdown.ts, shared API/refactor risk | src/outputs/policy-engine.ts contains 42 detected symbols, 8 imports, 8 exports. |
...
| Bucket | Tokens | Files |
| --- | --- | --- |
| Directly relevant source files | 580 | `benchmarks/tasks/refactor-config-loader.json`, `assets/agent-context-code-layers.png`, `src/outputs/markdown.ts`, `src/outputs/context-delta.ts`, `src/core/token-estimator.ts` |
| Tests | 2,109 | `test/context-delta.test.ts`, `benchmarks/fixtures/small-ts-app/test/api/login.test.ts`, `benchmarks/fixtures/monorepo/packages/api/test/config.test.ts`, `benchmarks/fixtures/fastapi-app/tests/test_users.py`, `benchmarks/fixtures/small-ts-app/test/auth/session.test.ts`, `benchmarks/fixtures/react-app/src/auth/useAuthState.test.ts`, `benchmarks/fixtures/react-app/src/components/LoginStatus.test.tsx`, `benchmarks/fixtures/monorepo/packages/config/test/loader.test.ts`, `test/contract-validator.test.ts`, `test/execution-trace.test.ts`, `test/freshness.test.ts`, `test/loop-controller.test.ts`, `test/policy-engine.test.ts`, `test/task-harness.test.ts`, `test/test-selector.test.ts`, `test/agents-md.test.ts`, `test/config.test.ts`, `test/mcp.test.ts`, `test/retrievers.test.ts`, `test/writer.test.ts`, `test/analyzers.test.ts`, `test/cache.test.ts`, `test/impact.test.ts`, `test/task-context.test.ts`, `test/fixtures.test.ts`, `test/token-savings.test.ts`, `test/readiness.test.ts`, `test/validator.test.ts`, `test/scanner.test.ts`, `test/snapshot.test.ts`, `test/benchmark.test.ts` |
| Dependency neighbors | 6,395 | `src/outputs/rag.ts`, `src/core/types.ts`, `src/outputs/task-context.ts`, `src/outputs/agents-md.ts`, `src/core/freshness.ts`, `src/outputs/task-harness.ts`, `src/outputs/contract-validator.ts`, `src/outputs/execution-trace.ts`, `src/outputs/task-run.ts`, `src/outputs/loop-controller.ts`, `src/outputs/policy-engine.ts`, `src/core/context-builder.ts`, `src/outputs/writer.ts`, `src/outputs/impact.ts`, `src/outputs/test-selector.ts`, `src/benchmarks/benchmark.ts`, `src/retrievers/index.ts`, `src/core/cache.ts`, `src/core/scanner.ts`, `src/core/token-savings.ts`, `src/outputs/architecture.ts`, `src/outputs/dependency-graph.ts`, `src/outputs/context-layers.ts`, `src/outputs/key-files.ts`, `src/outputs/module-map.ts`, `src/outputs/onboarding.ts`, `src/outputs/repo-summary.ts`, `src/outputs/token-savings.ts`, `src/outputs/readiness.ts`, `src/core/git.ts`, `src/retrievers/static.ts`, `src/core/path-utils.ts`, `src/retrievers/types.ts`, `src/config/load-config.ts`, `src/core/indexer.ts`, `src/analyzers/python.ts`, `src/analyzers/javascript.ts`, `src/retrievers/ripgrep.ts` |
...
| Entrypoints | 572 | `src/cli/index.ts`, `src/mcp/server.ts` |

Remaining budget: 1,668 estimated tokens
